...
import { Heart, Plus, Send, CornerDownRight, Shield, MessageSquare } from 'lucide-react';
...
const StoryCard: React.FC<StoryCardProps> = ({ story, onUpdate }) => {
  ...
  // Esoteric timestamp simulation
  const getEsotericTime = (date: string) => {
    const d = new Date(date);
    const hours = d.getHours();
    let phase = '';
    if (hours >= 5 && hours < 8) phase = 'during the Golden Dawn';
    else if (hours >= 8 && hours < 11) phase = 'during the Solar Ascent';
    else if (hours >= 11 && hours < 14) phase = 'at the Solar Zenith';
    else if (hours >= 14 && hours < 17) phase = 'during the Solar Descent';
    else if (hours >= 17 && hours < 20) phase = 'at the Twilight Hour';
    else if (hours >= 20 && hours < 23) phase = 'during the Lunar Rise';
    else phase = 'in the Stillness of Night';

    const timeAgo = Math.floor((new Date().getTime() - d.getTime()) / (1000 * 60 * 60));
    const timeStr = timeAgo === 0 ? 'Recently' : `${timeAgo}h ago`;
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-ui uppercase tracking-widest text-resonance-gold/70">
              {getEsotericTime(story.created_at)}
            </p>
          </div>
        </div>
